...
<CurrencyList>
    ...
    /**
     * Converts number to currency. Usage: val moneyToStr = new MoneyToStr("UAH", "UKR", "TEXT"); val result =
...
object MoneyToStr {
    ...
    def main(args: Array[String]): Unit = {
        var amount: String = "123.25"
        var language: String = "ENG"
        var currency: String = "USD"
        var pennies: String = "TEXT"
        if (args.length == 0) {
            print("Usage: sbt \"run --amount=123.25 --language=rus|ukr|eng --currency=rur|uah|usd --pennies=text|number\"");
        } else {
            for (arg: String <- args) {
            ...
            val result: String = new MoneyToStr(currency, language, pennies).convert(amount.toDouble)
            print(result)
        }
    }
}
